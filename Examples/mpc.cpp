...
int main() {
    auto predict = [](auto x, auto u, auto) { return x * 2.0 - u; };

    std::array<double, HORIZ> trajectory{
            1, 1, 5, 1, 1,
    };

    auto cost = [trajectory](auto x, auto, auto t) { return (x - trajectory[t]) * (x - trajectory[t]); };

    auto term = [](auto err) { return err < 0.1; };
